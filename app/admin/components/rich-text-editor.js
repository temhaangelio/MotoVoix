...
function ToolButton({ label, shortcut, onPress, children }) {
  return (
    <button
      type="button"
      aria-label={label}
      ...
      onClick={onPress}
      className="grid size-9 shrink-0 place-items-center rounded-full text-white transition-colors hover:bg-white/15 focus-visible:outline-white"
    >
      {children}
    </button>
  );
...
export function RichTextEditor({ id, name, value, onChange, onBlur }) {
  ...
  function syncValue() {
    if (editorRef.current) onChange(editorToMarkdown(editorRef.current));
  }

  function command(commandName, commandValue) {
    editorRef.current?.focus();
  ...
  const frame = (
    <div
      className={cn(
        fullscreen
          ? "fixed inset-0 z-[200] flex h-[100dvh] w-screen flex-col overflow-hidden bg-white"
    ...
    >
      ...
      <div
        ...
        onPaste={handlePaste}
        className={cn(
          "px-5 pb-5 pt-[76px] text-[16px] leading-7 text-[#272727] outline-none [&_a]:underline [&_a]:underline-offset-4 [&_blockquote]:border-l-2 [&_blockquote]:border-black [&_blockquote]:pl-4 [&_blockquote]:italic [&_code]:rounded [&_code]:bg-[#e8e8e8] [&_code]:px-1.5 [&_h1]:mb-3 [&_h1]:text-3xl [&_h1]:font-bold [&_h2]:mb-2 [&_h2]:text-2xl [&_h2]:font-bold [&_mark]:rounded-[3px] [&_mark]:bg-[#eaeaea] [&_mark]:px-0.5",
          fullscreen
            ? "mx-auto min-h-0 w-full max-w-5xl flex-1 overflow-y-auto px-8 pb-20 pt-24 text-[18px] leading-8 sm:px-16"
            : "min-h-[360px]",
        )}
      />
    </div>
  ...
  return (
    <>
      ...
      {fullscreen && mounted ? createPortal(frame, document.body) : frame}
      {fullscreen ? <div className="min-h-[360px] rounded-2xl bg-[#f5f5f5]" aria-hidden="true" /> : null}
    </>
  );
}
